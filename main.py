...
def get_combination_option(options_file_path, last_close, day, return_on_inves_thre=0.02):
    stock_name = options_file_path.split(os.sep)[-1][:-4]
    try:
...
def main():
   #     Stock_name='AMD'
    keys = ['market', 'stock_symbol', 'last_close' , 'etf_symbol', 'correlation', 'tech_id', 'trigging_tech_idx', 'strategy', 'DTE', 'strike', 'inout', 'IV_p/IV_c/HV', 'BBlower_Close', 'BBupper_Close']
    for stock_name in open('stock_num.txt', 'r').readlines():
        # if 'ITW' in stock_name or 'CNP' in stock_name: #  or 'ITW' in stock_name
        #     pass
        # else:
        #     continue
        ...
        try:
            Stock_name=Stock_name.replace(".","-") # BRK.B
        except:
            pass
        output['stock_symbol'] = Stock_name
        output['etf_symbol'] = stock_name.split('-')[1]
        A=Stock_history.sum()
        ...
        for k in condition_result.keys():
            if condition_result[k][f'in{k[:2]}'][1]: # check triggered or not
                output['strategy'] = k#k[:2]
                output['inout'] = 'in'
                output['trigging_tech_idx'] = condition_result[k][f'in{k[:2]}']
                break
            if condition_result[k][f'out{k[:2]}'][1]: # check triggered or not
                output['strategy'] = k#k[:2]
                output['inout'] = 'out'
...
#                print ('out', condition_result[k][f'out{k[:2]}'][1])
        ...
        if output['inout'] == 'in':
            if 'BC' == output['strategy']:
                tech_id_L = [28,26] # 白上跨藍: 考慮周,不考慮周
            elif 'BC_S' == output['strategy']:
                tech_id_L = [41,12,11,10,20,19,18,40,4,3,2,9,17,1] # 依照經驗排序
            elif 'BP_S' == output['strategy']:
                tech_id_L = [42,16,15,14,24,23,22,8,7,6,13,21,5] # 依照經驗排序
            elif 'BP' == output['strategy']:
                tech_id_L = [29,27] # 白下跨藍: 考慮周,不考慮周
            elif 'SP' == output['strategy']:
                tech_id_L = [32,36,38,34,30] # 正三合一: W/D牛市,D牛市,D熊市,W/D熊市,不分市場
            elif 'SC' == output['strategy']:
                tech_id_L = [33,37,39,35,31] # 反三合一: W/D牛市,D牛市,D熊市,W/D熊市,不分市場
            else:
                assert False, 'error in tech_id'
            out=0
            for tech_id in tech_id_L:
                if out==1: break
                for key_T in daily_dict['trigger']:
                    temp=int(key_T.split("/")[0].split("_")[1])
                    if tech_id==temp:
                        try:
                            bt_result = p[f'{tech_id}/Qa/Ma'].split(',')[-1]
                            output['tech_id'] = f'{tech_id}/Qa/Ma:' + p[f'{tech_id}/Qa/Ma']
                            out=1
                            break
                        except:
                            month_=dt.today().month#.strftime("%Y-%m-%d")
                            if month_<=3:
                                Q=1
                            elif month_>3 and month_<=6:   
                                Q=2 
                            elif month_>6 and month_<=9:   
                                Q=3
                            else:   
                                Q=4
                            try:
                                bt_result = p[f'{tech_id}/Q{Q}/Ma'].split(',')[-1]
                                output['tech_id'] = f'{tech_id}/Q{Q}/Ma: ' + p[f'{tech_id}/Q{Q}/Ma']   
                                out=1
                                break  
                            except:
                                pass
...
#        print ('best_ombination_option: ', best_ombination_option)
        ...
        if date!=0:
            ...
            iv_delta_price = (pow(float(detla_d)/365, 0.5)) * last_close * float(IV['HV']['30day']/100)
        ...
        if iv_new_price=='NAN':
            ...
            output['strike'] = round(iv_new_price,2)
        F = BB.sum()
        BB_dict=F.bollinger_bands(df, DTE=60, lookback=20, numsd=2) # price,DTE,BB中心均線(fix),內BB標準差
        print (BB_dict)
        output['BBupper_Close'] = round((BB_dict['upper_in'] - last_close) / last_close*100,2)
        output['BBlower_Close'] = round((last_close - BB_dict['lower_in']) / last_close*100,2)
        print ('output: ', output)
